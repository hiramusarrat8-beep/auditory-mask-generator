...
class PresetManager:
    ...
    def _widget_names(self):
        return {
            "line_edits": [
                ...
                "mondrian_tone_duration_input", "mondrian_carrier_min_input", "mondrian_carrier_max_input",
                "mondrian_prf_min_input", "mondrian_prf_max_input", "mondrian_duty_cycle_input",
                "pulse_start_input",
            ],
            "combos": [
                "ramp_shape_combo", "bg_type_combo", "bg_ramp_shape", "playback_mode_combo",
                "colored_noise_combo",
            ],
            "checkboxes": [
    ...
    def apply_preset_data(self, preset_data):
        ...
        # enable_dual_checkbox must be restored first: its handler (on_toggle_dual)
        # rebuilds the playback_mode_combo item list and hard-sets the current text.
        # Restoring the checkbox before the combos ensures playback_mode_combo already
        # has the correct items when we set it below.
        checkboxes = ui_state.get("checkboxes", {})
        dual_widget = getattr(self.gui, "enable_dual_checkbox", None)
        if dual_widget is not None and "enable_dual_checkbox" in checkboxes:
            dual_widget.setChecked(bool(checkboxes["enable_dual_checkbox"]))

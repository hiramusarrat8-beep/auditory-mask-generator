...
def generate_auditory_mondrian_mask(duration, sr, density=8, tone_duration=0.5,
                                    carrier_min=3000, carrier_max=15000,
    ...
    carrier_low, carrier_high = sorted((carrier_min, carrier_max))
    prf_low, prf_high = sorted((prf_min, prf_max))

    for _ in range(tones):
        t = np.arange(tone_samples) / sr
        carrier_freq = np.random.uniform(carrier_low, carrier_high)
        carrier = np.sin(2 * np.pi * carrier_freq * t)
...
def generate_continuous_background(duration, fs, bg_type, carrier_freq=0, bg_ramp_shape="None", bg_ramp_length=0.0,
    ...
    if bg_type == "Narrowband Noise":
        ...
        bg = generate_colored_noise(duration, fs, colored_noise_settings.get("color", "Pink"))
        ...
        bg = generate_auditory_mondrian_mask(
            ...
            carrier_min=mondrian_settings.get("carrier_min", 20),
            carrier_max=mondrian_settings.get("carrier_max", 15000),
